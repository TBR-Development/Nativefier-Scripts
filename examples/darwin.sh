...
wait 1
set AppName = Example 
set AppTitle = example 
set AppUrl = https://www.example.com/
set InternalUrls = (.*?)(*.example.*|*.example.*)(.*?)
echo "======================================"
...
echo "======================================"
wait 1
nativefier -v -n "%AppTitle%" "%AppUrl%" --tray --bounce --enable-es3-apis --file-download-options "%FileDownloadOptions%" --internal-urls "%InternalUrls%" "%BuildPath%/%AppName%" > "%LogPath%/%LogFile%"
endlocal
